...
export async function getDefaultLocation(admin) {
  // Usar location_id fijo del env para evitar llamada API con scope read_locations
  if (process.env.SHOPIFY_LOCATION_ID) {
    return parseInt(process.env.SHOPIFY_LOCATION_ID);
  }
...
export async function syncBundleInventory(variantId, quantitySold, locationId, bundleConfig, admin) {
  ...
  for (const component of bundleConfig.components) {
    ...
    try {
      ...
      console.log(`   ✅ Adjusted: new available = ${result.available}`);
      results.push({
      ...
      console.error(`   ❌ Error ${component.product_title}: ${error.message}`);
      results.push({
        variant_id: component.variant_id,
        product_title: component.product_title,
        success: false,
